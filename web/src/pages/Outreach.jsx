...
import { Mail, Ghost, RefreshCw, BarChart2, ChevronDown, ChevronUp, Search, X } from 'lucide-react'
import { api } from '../api'
...
export default function OutreachPage() {
  ...
  const filtered = searchQ.trim()
    ? shown.filter(r => {
        const company = (companyMap[r.company_id]?.name || '').toLowerCase()
        const contact = (contactMap[r.contact_id]?.name || '').toLowerCase()
  ...
  return (
    <div className="flex flex-col min-h-screen bg-app">
      ...
      <div className="px-4 pt-3 pb-1 relative">
        <Search size={14} className="absolute left-7 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
        <input
          type="text"
          placeholder="Search company or contact…"
          value={searchQ}
          onChange={e => setSearchQ(e.target.value)}
          className="w-full pl-8 pr-8 py-2 border border-theme rounded-xl text-sm bg-card text-body"
        />
        {searchQ && (
          <button onClick={() => setSearchQ('')} className="absolute right-7 top-1/2 -translate-y-1/2 text-muted">
            <X size={14} />
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="text-center text-muted text-sm py-12">
            {searchQ ? `No results for "${searchQ}"` : tab === 'active' ? 'No active outreach' : 'No ghosted contacts'}
          </div>
        ) : (
          filtered.map(r => (
            <OutreachCard
